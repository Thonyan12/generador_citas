...
\item \textbf{Parámetros:} \texttt{$key` (String), `$lang} (String, opcional, default \texttt{'es'}).
...
\item \textbf{Parámetros:} \texttt{$author` (Array), `$format} (String).
...
\item \textbf{Concatenación de Nombres:} Junta \texttt{$firstName` y `$middleName} en \texttt{\$full\_first}.
\item \textbf{Concatenación de Apellidos:} Junta \texttt{$lastName` y `$secondLastName} en \texttt{\$full\_last}.
\item \textbf{Formateo de Iniciales (para estilos como APA / Vancouver):}
\begin{itemize}
\item Extrae la primera letra de \texttt{$firstName` usando `mb_substr($first, 0, 1, 'UTF-8')} y le añade un punto.
...
\item Se añade únicamente el último autor de la lista (\texttt{$authors_list[$count - 1]}).
\end{itemize}
\end{enumerate}
\subsection{Algoritmo de Truncamiento para IEEE (7+ Autores)}
\begin{enumerate}
\item Contar los autores en la lista \texttt{$count = count($authors)}.
\item Si \texttt{$count >= 7`, toma únicamente el primer autor formateado (`$authors\_list[0]}) y le concatena la locución latina \texttt{" et al."}.
\end{enumerate}
